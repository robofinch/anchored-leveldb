...
#[test]
fn empty_list() {
    let list = SimpleSkiplist::new(DefaultComparator);
...
fn empty_list_lending_iter() {
    ...
    let list = SimpleSkiplist::from_lending_iter(*iter);
...
fn extend_iter_reference() {
    ...
    let mut list: SimpleSkiplist<DefaultComparator> = Default::default();
    list.insert_copy(&[0]);
    ...
    // `anchored_skiplist::simple::Iter::current`, the safety guarantees made by `Iter` apply,
    // and since the backing skiplist is not invalidated until the end of this function
    // (aside from being moved), extending the lifetime to the end of this function is sound.
...
fn extend_lending_iter_reference() {
    ...
    // `LendingIter` apply, and since the backing iterator (and skiplist inside) is not invalidated
    // until the end of this function (aside from being moved), extending the lifetime to the end
    // of this function is sound.
    let data: &[u8] = unsafe { &*data };
    let _iter = iter;
...
#[test]
fn two_element_list() {
    let mut list = SimpleSkiplist::new_seeded(DefaultComparator, 5);
    ...
    let list = SimpleSkiplist::write_unlocked(locked);
...
fn two_element_list_lending_iter() {
    ...
    let mut iter = SimpleSkiplist::from_lending_iter(iter).lending_iter();
    assert!(!iter.is_valid());
...
fn wrapped_comparators() {
    let mut list: SimpleSkiplist<GenericContainer<DefaultComparator, Box<DefaultComparator>>>
        = SimpleSkiplist::new(GenericContainer::new(Box::new(DefaultComparator)));
    ...
    let mut list: SimpleSkiplist<Rc<dyn Comparator>>
        = SimpleSkiplist::new(Rc::new(DefaultComparator));
...
fn broken_comparators() {
    ...
    let mut list: SimpleSkiplist<BadComparator> = SimpleSkiplist::new(BadComparator::new());
...
fn many_insertions_and_reads() {
    // We will randomly choose 2048 (possibly-repeated) entries, out of 4096 possible values.
    // Those possible entries are slices of length 4, each of whose bytes are strictly less than
    // 8. (And 8^4 == 4096.)
    ...
    let mut skiplist = SimpleSkiplist::new(DefaultComparator);
    // `sorted_entries` will have the correct order of entries, because `DefaultComparator`
    // uses `Ord`. Note that the `skiplist` and `BTreeMap` handle duplicate entries identically...
    // aside from `skiplist` not deallocating wasted memory immediately.
    ...
    for entry_num in 0..total_possible_entries {
        ...
        skiplist = SimpleSkiplist::from_lending_iter(lending_iter);
    }
    ...
    let skiplist = SimpleSkiplist::from_lending_iter(lending_iter);
    ...
    // Note that `next` was called one more time on `std_iter` than `prev` was on `lending_iter`.
    iter.prev();
    assert!(!iter.is_valid());
