...
pub struct MaybeSyncArc<const SYNC: bool, T: ?Sized> {
    /// # Safety invariant
    /// If `SYNC` is true, then the `sync` field of this union is active; otherwise,
    /// the `unsync` field is active.
...
/// This type is equivalent to [`alloc::sync::Weak<T>`] if `SYNC` is `true`, and it is otherwise
/// equivalent to [`alloc::rc::Weak<T>`].
///
/// The type is implemented with a union.
///
/// # Guarantees for `unsafe` code
