...
use anchored_vfs::{CreateParentDir, LevelDBFilesystem, SyncParentDir, WritableFile as _};

use crate::pub_typed_bytes::FileNumber;
...
pub(crate) fn set_current<FS: LevelDBFilesystem>(
    filesystem:           &FS,
    db_directory:         &Path,
    manifest_file_number: FileNumber,
    manifest_name:        &str,
) -> Result<(), (FilesystemError<FS::Error>, SetCurrentError)> {
    /// Used for a `try` scope.
    fn perform_writes<FS: LevelDBFilesystem>(
        filesystem:    &FS,
        db_directory:  &Path,
        manifest_name: &str,
        temp_path:     &Path,
        ...
        temp_file.write_all(manifest_name.as_bytes())
            .map_err(|io_err| (FilesystemError::Io(io_err), SetCurrentError::WriteTemp))?;
        temp_file.write_all(b"\n")
            .map_err(|io_err| (FilesystemError::Io(io_err), SetCurrentError::WriteTemp))?;

        temp_file.sync_data()
            .map_err(|io_err| (FilesystemError::Io(io_err), SetCurrentError::SyncTemp))?;

        filesystem.rename(
                temp_path,
                &LevelDBFileName::Current.file_path(db_directory),
                SyncParentDir::True,
            )
            .map_err(|fs_err| (
                FilesystemError::FsError(fs_err),
                SetCurrentError::RenameTempToCurrent,
            ))?;
    ...
    let temp_file = filesystem
        .open_writable(&temp_path, CreateParentDir::False, SyncParentDir::False)
        .map_err(|fs_err| (FilesystemError::FsError(fs_err), SetCurrentError::OpenTemp))?;
    ...
    if let Err(error) = perform_writes(
        ...
        let _err = filesystem.remove_file(&temp_path);
        return Err(error);
    }
