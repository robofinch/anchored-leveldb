...
/// # Safety invariant
/// The inner value of a [`Level`] is strictly less than [`NUM_LEVELS`].
...
impl Level {
    ...
    // Safety invariant: `NUM_LEVELS_USIZE` is strictly greater than anything from 0 to 6.
    ...
    pub const fn next_level(self) -> Option<Self> {
        ...
        if self.0 + 1 < NUM_LEVELS {
            // Safety invariant: we checked the invariant.
            Some(Self(self.0 + 1))
        } else {
            None
        }
    ...
    pub fn all_levels() -> impl ExactSizeIterator<Item = Self> + DoubleEndedIterator {
        // Safety invariant: the upper bound of the range is exclusive
    ...
    pub fn middle_levels() -> impl ExactSizeIterator<Item = Self> + DoubleEndedIterator {
        // Safety invariant: the subtraction does not overflow, so the upper bound of the range
        // does not include `NUM_LEVELS`.
    ...
    pub fn inclusive_range(
        ...
        // Safety invariant: the upper bound of the range is strictly less than `NUM_LEVELS` by the
...
impl<T> IndexLevel<T> for [T; NUM_LEVELS_USIZE] {
        ...
        T: 'a,
    {
        self.iter().fuse().enumerate().map(|(index, value)| {
            // Safety invariant: the iter returns the `NUM_LEVELS_USIZE` elements in the array, and
