...
use crate::{
    pub_typed_bytes::{EntryType, FileNumber, SequenceNumber},
...
pub(crate) struct OwnedSortedFiles(Vec<Arc<FileMetadata>>);
...
impl OwnedSortedFiles {
    ...
    pub fn merge<Cmp: LevelDBComparator>(
        ...
        added.sort_unstable_by(move |lhs, rhs| {
            cmp.cmp(lhs.smallest_key(), rhs.smallest_key())
        });
        ...
        for added_file in added.iter() {
            ...
            'inner: while let Some(file) = base_file {
                if cmp.cmp(file.smallest_key(), added_file.smallest_key()).is_lt() {
                    if !deleted.contains(&file.file_number()) {
                        merged_files.push(Arc::clone(file));
                    }
            ...
            if !deleted.contains(&added_file.file_number()) {
                merged_files.push(Arc::clone(added_file));
            }
        }
        ...
        if let Some(base_file) = base_file {
            ...
            if !deleted.contains(&base_file.file_number()) {
                merged_files.push(Arc::clone(base_file));
            }
            ...
            for remaining_base_file in base_files {
                if !deleted.contains(&remaining_base_file.file_number()) {
                    merged_files.push(Arc::clone(remaining_base_file));
                }
            }
        }
    ...
    #[inline]
    #[must_use]
    pub fn borrowed(&self) -> SortedFiles<'_> {
        SortedFiles(&self.0)
    }
}
...
pub(crate) struct SortedFiles<'a>(
    &'a [Arc<FileMetadata>],
);
...
impl<'a> SortedFiles<'a> {
    #[inline]
    #[must_use]
    pub const fn inner(self) -> &'a [Arc<FileMetadata>] {
        self.0
    }
    ...
    pub fn find_file_disjoint<Cmp: LevelDBComparator>(
        ...
        match self.0.binary_search_by(|file| cmp.cmp(file.largest_key(), key)) {
