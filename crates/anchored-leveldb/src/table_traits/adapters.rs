...
/// [`TableComparator`], [`SSTableFilterPolicy`], and so on) suffice, we can consider the four
...
/// This type fulfills the semantic constraints of [`SSTableFilterPolicy`], in addition to
...
/// `InternalComparator<Cmp>` restricted to valid internal keys is the same as strict equality.
///
/// See [`SSTableFilterPolicy::key_may_match`].
#[derive(Debug, Clone, Copy)]
