...
use super::{
    ...
    file::{ThreadsafeMemoryFile, ThreadsafeFileInner},
...
pub struct ThreadsafeMemoryFS {
    directories: Mutex<HashSet<PathBuf>>,
    files:       Mutex<HashMap<PathBuf, ThreadsafeFileInner>>,
    locks:       Locks,
}

impl ThreadsafeMemoryFS {
    /// Create an empty `ThreadsafeMemoryFS`, with no files or directories.
    #[inline]
    #[must_use]
    pub fn new() -> Self {
        Self::default()
    ...
    /// extremely likely to trigger a deadlock. To avoid such an access and deadlock, using any
    /// `ThreadsafeMemoryFS`-related functions or structs should be avoided inside the callback.
    ///
    /// Because this function takes `&mut self`, the callback cannot (without UB) use a reference to
    /// the `ThreadsafeMemoryFS` itself, but a pathological callback could capture a `MemoryFile`
    /// referencing the file at `path`.
    ...
    /// Iterate over the path of every file in the `ThreadsafeMemoryFS`. The order of paths is not
    /// consistent.
    ///
    /// # Errors
    /// Returns a [`MutexPoisoned`] error if an internal mutex was poisoned.
    pub fn file_paths(&mut self) -> Result<impl Iterator<Item = &Path>, MutexPoisoned> {
        Ok(
            self.files
                .get_mut()?
                .keys()
    ...
    /// Iterate over the path of every directory in the `ThreadsafeMemoryFS`. The order of paths is
    /// not consistent.
    ///
    /// # Errors
    /// Returns a [`MutexPoisoned`] error if an internal mutex was poisoned.
    pub fn directory_paths(&mut self) -> Result<impl Iterator<Item = &Path>, MutexPoisoned> {
        Ok(
            self.directories
                .get_mut()?
                .iter()
    ...
    /// Iterate over every file in the `ThreadsafeMemoryFS`. The order in which files are returned
    /// is not consistent.
    ///
    /// # Errors
    /// Returns a [`MutexPoisoned`] error if an internal mutex was poisoned.
    pub fn files(&mut self) -> Result<
        impl Iterator<Item = (&Path, ThreadsafeMemoryFile)>,
        MutexPoisoned,
    > {
        Ok(
            self.files
                .get_mut()?
                .iter()
                .map(|(path, inner)| (&**path, ThreadsafeMemoryFile::open(inner)))
        )
    }

    /// Mutably access the buffers backing each file in the `ThreadsafeMemoryFS`. The order in
    /// which files are accessed is not consistent.
    ///
    /// If the provided callback returns an error or an internal mutex is poisoned, iteration is
    /// halted early, and the error is returned.
    ///
    /// # Deadlocks
    /// If the provided callback accesses the same file via a different handle, the callback is
    /// extremely likely to trigger a deadlock. To avoid such an access and deadlock, using any
    ...
    where
        ...
        Err: From<MutexPoisoned>,
    {
        self.files
            .get_mut().map_err(MutexPoisoned::from)?
            .iter()
...
impl ThreadsafeMemoryFS {
    ...
    where
        F: FnOnce(&ThreadsafeFileInner) -> Result<T, Error>,
    {
        if let Some(file) = self.files.lock()?.get(&path) {
            callback(file)

        } else if self.directories.lock()?.contains(&path) {
    ...
    fn assert_directory_exists(&self, path: &Path) -> Result<(), Error> {
        if self.directories.lock()?.contains(path) {
            Ok(())
    ...
    where
        F: FnOnce(&mut ThreadsafeFileInner) -> Result<T, Error>,
    {
        if self.directories.lock()?.contains(&path) {
        ...
        let file = files
            .entry(path)
            .or_insert(ThreadsafeFileInner::new());

        callback(file)
    }
}

impl ReadableFilesystem for ThreadsafeMemoryFS {
    type ReadFile               = ThreadsafeMemoryFile;
    type RandomAccessFile       = ThreadsafeMemoryFile;
    type Error                  = Error;
    ...
    fn open_sequential(&self, path: &Path) -> Result<Self::ReadFile, Self::Error> {
        ...
        self.access_file_inner(path, |file| {
            Ok(ThreadsafeMemoryFile::open(file))
        })
    }
    ...
    fn exists(&self, path: &Path) -> Result<bool, Self::Error> {
        ...
        Ok(self.files.lock()?.contains_key(path) || self.directories.lock()?.contains(path))
    }
    ...
    fn children(&self, path: &Path) -> Result<Self::DirectoryChildren, Self::Error> {
        ...
        let children = self.files.lock()?.keys()
            .chain(self.directories.lock()?.iter())
...
impl WritableFilesystem for ThreadsafeMemoryFS {
    type WriteFile  = ThreadsafeMemoryFile;
    type AppendFile = ThreadsafeMemoryFile;
    ...
    fn open_writable(
        ...
        self.create_inner_file(path, create_dir, |file| {
            Ok(ThreadsafeMemoryFile::open_and_truncate(file)?)
        })
    }
    ...
    fn open_appendable(
        ...
        self.create_inner_file(path, create_dir, |file| {
            Ok(ThreadsafeMemoryFile::open_append(file)?)
        })
    }
    ...
    fn remove_dir(&self, path: &Path) -> Result<(), Self::Error> {
        ...
        if has_children {
            ...
            self.directories.lock()?.remove(path);
            Ok(())
        }
    }
