...
pub(super) struct Locks(Mutex<Vec<PathBuf>>);
...
impl Locks {
    ...
    /// already locked (and an internal mutex is not poisoned).
    ...
    /// This method only fails for such pathological uses, or if an internal mutex is poisoned.
    ///
...
impl<T> From<PoisonError<T>> for LockError {
    ...
    fn from(_err: PoisonError<T>) -> Self {
        Self::FSError(Error::MutexPoisoned)
    }
}
